...
public class Auth {
    ...
    private String TAG = "Authenticator_Lockscreen_Auth";
    private Gson gson;
    ...
    public String auth(final ASMRequestAuth asmRequestAuth) {
        try {
            String keyID = getKeyId(sharedPreferences, asmRequestAuth);
            ...
            byte[] signatureTag = getSignatureTag(keyID, uafV1SignedData);
    ...
    public static String getKeyId(SharedPreferences sharedPreferences, final ASMRequestAuth asmRequestAuth) {
        String keyID;
    ...
    private byte[] getSignatureTag (String keyID, byte[] signedData) throws IOException {
        ByteArrayOutputStream outputStream = new ByteArrayOutputStream();
        outputStream.write(UnsignedUtil.encodeInt(TagsEnum.TAG_SIGNATURE.id));
        byte[] value = Crypto.getSignatureForKeyID(keyID, signedData);
        outputStream.write(UnsignedUtil.encodeInt(value.length));
        outputStream.write(value);
